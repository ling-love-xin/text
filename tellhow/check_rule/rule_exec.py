...
WHERE   tab_a.chk_id = {tab_rule_info.chk_id})""", con=pg_con)
    for index, row in data_bat_new[(data_bat_new.db_nm == tab_rule_info.chk_db) & (
        ...
        value = (
            ...
            err_cont.head(1000).to_json(force_ascii=False, orient='table', index=False),
            # err_cont.to_json(force_ascii=False,orient='table',index=False),
            begin_time,
            end_time,
            f"""对 {tab_rule_info.chk_obj_dec}({tab_rule_info.chk_obj})表 的增量进行{tab_rule_info.rule_name}检核，其中错误数据量为{str(err_cont.shape[0])}，检查总量为{str(all_cont.shape[0])}，检核字段为{tab_rule_info.chk_clo}。""",
        )
        values.append(value)
    return values
...
def get_all_values(tab_rule_info: pd.DataFrame):
    ...
    for index, row in tab_rule_info.iterrows():
        ...
        value = (
            ...
            begin_time,
            end_time,
            f"""对 {tab_rule_info.chk_obj_dec}({tab_rule_info.chk_obj})表 的全量进行{tab_rule_info.rule_name}检核，其中错误数据量为{str(err_cont.shape[0])}，检查总量为{str(all_cont.shape[0])}，检核字段为{tab_rule_info.chk_clo}。""",
        )
        values.append(value)
    return values
...
if __name__ == '__main__':
    if len(sys.argv) != 1:
        ...
        table_rule_info = chk_cfg_data[(chk_cfg_data.chk_obj == chk_obj)&(chk_cfg_data.chk_db == chk_db)]
