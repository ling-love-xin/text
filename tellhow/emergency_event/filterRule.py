...
import pandas as pd
import re
import emergency_event.conf.rules
import json


# 从所有数据中挑选出可能出现人员伤亡的类别
from conf import rules


def get_target_category_from_emergency(
        df: 'Dataframe',
        c_first: list,
        c_second: list):
    # 将可能造成人员伤亡的事件类型筛选出来
    target_df = df[(df['emcy_evt_ctgy_nm'].isin(c_first)) &
                   (df['emcy_evt_typ_nm'].isin(c_second))]
    # 将没有造成人员伤亡的事件筛选出来写入到输出结果中
    for i, row in enumerate(df.iterrows()):
        row = row[1]
        if row['evt_memo'] not in list(target_df['evt_memo']):
            df.loc[df.evt_id == row['evt_id'], ['dead_pers_cnt',
                                                'seri_injure_pers_cnt', 'minr_injure_pers_cnt']] = (0, 0, 0)
    return target_df


def get_possible_people_hurt_emergency(
        df_possible: 'Dataframe',
        pattern_possible_str: str,
        pattern_no_hurt_str: str,
        num_analysis_result: list):
    emergency = df_possible['evt_memo']
    # 设定 可能出现人员伤亡的正则表达式，选出事件描述中包含这些字的
    pattern_possible = re.compile(pattern_possible_str)
    # 设定 没有人员伤亡的正则表达式，从事件描述中匹配 无...伤亡 || 未...受伤 || 无... 受伤 || 未...伤亡
    pattern_no_hurt = re.compile(pattern_no_hurt_str)
    people_hurt = []

    for i, row_ in enumerate(df_possible.iterrows()):
        row = row_([1])['evt_memo']
        if pattern_possible.search(row):
            # 有一些特殊情况，需要单独写一下
            if '全部出院' in row:
                num_analysis_result.append([row, '0', '0', '0'])
                df_possible.loc[df_possible.evt_id == row_([1])['evt_id'], ['dead_pers_cnt',
                                                    'seri_injure_pers_cnt', 'minr_injure_pers_cnt']] = (0, 0, 0)
                continue
            if not pattern_no_hurt.search(
                    row) or '抢救无效死亡' in row or '生命危险' in row:
                people_hurt.append(row)
            # 如果事件描述中有未造成伤亡之类的，伤亡人数为0，直接写入到输出结果中
            else:
                num_analysis_result.append([row, '0', '0', '0'])
                df_possible.loc[df_possible.evt_id == row['evt_id'], ['dead_pers_cnt',
                                                                      'seri_injure_pers_cnt',
                                                                      'minr_injure_pers_cnt']] = (0, 0, 0)
        else:
            num_analysis_result.append([row, '0', '0', '0'])
            df_possible.loc[df_possible.evt_id == row['evt_id'], ['dead_pers_cnt',
                                                                  'seri_injure_pers_cnt',
    ...
    #     if pattern_possible.search(item):
    #         # 有一些特殊情况，需要单独写一下
    #         if '全部出院' in item:
    #             num_analysis_result.append([item, '0', '0', '0'])
    #             continue
    #         if not pattern_no_hurt.search(
    #                 item) or '抢救无效死亡' in item or '生命危险' in item:
    #             people_hurt.append(item)
    #         # 如果事件描述中有未造成伤亡之类的，伤亡人数为0，直接写入到输出结果中
    #         else:
    #             num_analysis_result.append([item, '0', '0', '0'])
    #     else:
...
def write_into_txt(output_path: str, num_analysis_result: list):
    with open(output_path, 'w', encoding='utf-8') as file:
        for item_all in num_analysis_result:
            item = item_all[0]
            death_num = item_all[1]
            count_injury_h = item_all[2]
            count_injury = item_all[3]
            file.write('原始事件摘要：\n')
...
# def result_to_xlsx(result_list: list, output_xlsx_path: str):
#     workbook = xlsxwriter.Workbook(output_xlsx_path)
#     worksheet = workbook.add_worksheet('伤亡人数统计结果')
#     headings = ['事件描述', '死亡人数', '重伤人数', '轻伤人数']
#     row_num = 1
...
def main():
    ...
    # 设置解析结果导出到xlsx文件的路径
    # output_xlsx_path = rules['output_xlsx_path']

    # 定义一个List存储所有数据，用于将所有数据写入到excel
    # num_analysis_result = []
    # # 读取紧急事件数据
    # df = pd.read_excel(filepath, sheet_name=emergency_sheet_name)

    # 根据事件的类别对可能出现人员伤亡的事件进行初步的筛选
    emergency_data = get_target_category_from_emergency(
        df, c_first, c_second)
    # 对可能出现人员伤亡的事件进行进一步的筛选，从中选出确实产生了人员伤亡的事件
    people_hurt_data = get_possible_people_hurt_emergency(
        emergency_data,
        output_result_path,
        pattern_possible_str,
        pattern_no_hurt_str,
        num_analysis_result)
